...
def render_controlled_search_results_page(query: str) -> str:
    ...
    match_result = match_manual_amazon_affiliate(displayed_query)
    escaped_query = escape(displayed_query)
    if match_result.match_status == AmazonManualMatchStatus.ELIGIBLE and match_result.result is not None:
        ...
        return (
            ...
            f'<p class="pw-line">{escape(safe_result.title)}</p>'
            ...
            f'<p class="pw-disclosure">{escape(safe_result.disclosure)}</p>'
            f'<p class="pw-safe-note">{escape(safe_result.safe_note)}</p>'
    ...
    return (
        ...
        '<main class="pw-wrap"><section class="pw-card">'
        f'<h1 class="pw-line">Search results for: {escaped_query}</h1>'
        '<p class="pw-line">No approved Amazon result is available for this query yet.</p>'
